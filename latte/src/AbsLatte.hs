...
data TopDef' a
    ...
    | ClassDef a Ident [ClassProp' a]
    | ClassExtDef a Ident Ident [ClassProp' a]
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)

type ClassProp = ClassProp' BNFC'Position
data ClassProp' a
    = CAttribute a (Type' a) Ident
...
data Expr' a
    ...
    | EString a String
    | EAtr a (Expr' a) Ident
    | ECMethodApp a (Expr' a) Ident [Expr' a]
    | ENull a (Expr' a)
...
instance HasPosition ClassProp where
  hasPosition = \case
    CAttribute p _ _ -> p
    CMethod p _ _ _ _ -> p
